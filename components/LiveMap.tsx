...
export default function ManagerLiveMap({ selectedVehicleId, onSelectVehicle }: LiveMapProps) {
  ...
  useEffect(() => {
    ...
    map.current = new mapboxgl.Map({ container: mapContainer.current, style: 'mapbox://styles/mapbox/dark-v11', center: [-9.3235, 38.6826], zoom: 13 });

    map.current.on('load', () => {
      map.current!.addSource('manager-route', { type: 'geojson', data: { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: [] } } });
      map.current!.addLayer({ id: 'manager-route-line', type: 'line', source: 'manager-route', layout: { 'line-join': 'round', 'line-cap': 'round' }, paint: { 'line-color': '#22c55e', 'line-width': 5, 'line-opacity': 0.9 } });
    });

    supabase.from('vehicle_positions').select('*').then(({ data }) => data?.forEach(updateMarker));

    const trackingChannel = supabase.channel('manager-tracking').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'vehicle_positions' }, (payload) => updateMarker(payload.new)).subscribe();
    const deliveriesChannel = supabase.channel('manager-deliveries-updates').on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'deliveries' }, (payload) => {
    ...
    return () => { supabase.removeChannel(trackingChannel); supabase.removeChannel(deliveriesChannel); map.current?.remove(); map.current = null; };
  }, []);
  ...
  async function showVehicleData(vehicle_id: string) {
    ...
    if (delivery) {
      const query = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${pos.lng},${pos.lat};${delivery.dropoff_lng || -9.3000},${delivery.dropoff_lat || 38.7070}?geometries=geojson&access_token=${MAPBOX_TOKEN}`);
  ...
  useEffect(() => {
    if (selectedVehicleId && positionsRef.current[selectedVehicleId]) showVehicleData(selectedVehicleId);
  }, [selectedVehicleId]);
